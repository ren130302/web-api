...
public class AppManagedDownload {
	...
	static class VGetStatus
		...
		public SpeedInfo getSpeedInfo(VideoFileInfo dinfo) {
			SpeedInfo speedInfo = this.map.get(dinfo);
			if (speedInfo == null) {
				speedInfo = new SpeedInfo();
				speedInfo.start(dinfo.getCount());
				this.map.put(dinfo, speedInfo);
			}
			return speedInfo;
		}
		...
		@Override
		public void run() {
			List<VideoFileInfo> dinfoList = this.videoinfo.getInfo();
			...
			switch (this.videoinfo.getState()) {
				...
				case DONE:
					if (this.videoinfo instanceof YouTubeInfo) {
						YouTubeInfo i = (YouTubeInfo) this.videoinfo;
						System.out.println(this.videoinfo.getState() + " " + i.getVideoQuality());
					}
					else if (this.videoinfo instanceof VimeoInfo) {
						VimeoInfo i = (VimeoInfo) this.videoinfo;
						System.out.println(this.videoinfo.getState() + " " + i.getVideoQuality());
					}
					else {
						System.out.println("downloading unknown quality");
					}
					for (VideoFileInfo d : this.videoinfo.getInfo()) {
						SpeedInfo speedInfo = this.getSpeedInfo(d);
					...
					break;
				case ERROR:
					System.out.println(this.videoinfo.getState() + " " + this.videoinfo.getDelay());
				...
				case DOWNLOADING:
					...
					if (now - 1000 > this.last) {
						this.last = now;

						String parts = "";

						for (VideoFileInfo dinfo : dinfoList) {
							SpeedInfo speedInfo = this.getSpeedInfo(dinfo);
							...
							System.out.println(String.format("file:%d - %s %.2f %s (%s)", dinfoList.indexOf(dinfo), this.videoinfo.getState(), dinfo.getCount()
									/ (float) dinfo.getLength(), parts, formatSpeed(speedInfo.getCurrentSpeed())));
						}
					}
					break;
				default:
	...
	public static void main(String[] args) {
		...
		File path = new File("/Users/axet/Downloads/");
